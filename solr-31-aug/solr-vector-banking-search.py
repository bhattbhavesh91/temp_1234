...
def advanced_banking_search(query, num_results=10):
    ...
    solr_query = {
        ...
        'bq': f'{{!func}}product(payload(embedding_payload),vector({json.dumps(query_embedding)}))',
        'fl': '*,score,product(payload(embedding_payload),vector({json.dumps(query_embedding)})) as embedding_score',
        'sort': 'score desc, embedding_score desc'
    }
